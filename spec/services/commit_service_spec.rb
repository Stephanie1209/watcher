...
RSpec.describe CommitService, vcr: true do
  ...
  describe "when commit doesnt exist" do
    describe "recieves branch name" do
      before(:each) do
      ...
      it "should create a new commit associated to branch name" do
        expect  {
          ...
          expect(Commit.last.sha).to eq("6e521ddfc81cb5855cb3cf205af26ab437a14901")
        }.to change(Commit, :count).by(1)
      end

      it "verifies commit-branch association" do
        @service.creates_or_updates_commit
        expect(Commit.last.branch.name).to eq("dev")
      end
    end

    describe "doesnt recieve a branch name" do
      before(:each) do
      ...
      it "should create a new commit" do
        expect  {
          @service.creates_or_updates_commit
          expect(Commit.last.sha).to eq("0adae263ce441c4033f23a6d9286ba883c1110da")
        }.to change(Commit, :count).by(1)
      end
